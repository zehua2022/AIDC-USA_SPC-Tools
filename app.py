...
# ===== 固定欄位名稱 =====
COLUMNS = ["圈碼","程式位置","補充","類型","值","目標尺碼","量測值","上偏差","下偏差","補充","差異值","過切值"]

# --- regex（同你之前的規則，可沿用/調整）---
NO_ANY = re.compile(r'(?i)\bno\.?\s*(\d+)')
NO_HEADER_FULL = re.compile(r'(?i)^\s*no\.?\s*(\d+)\s*$')
REF_ANY = re.compile(r'(?i)\bref\b')
...
def tag_groups_and_sequence(df: pd.DataFrame, src_name: str) -> pd.DataFrame:
    ...
    current_group_num, current_group_type = None, None
    for i, val in enumerate(df["圈碼"]):
        s = "" if pd.isna(val) else str(val)
        m = NO_ANY.search(s)
        grp_num = m.group(1) if m else None
...
# ========== 關鍵：把上傳檔案存成 bytes，並快取處理 ==========
@st.cache_data(show_spinner=True)
def process_files(files_payload: list[dict]) -> tuple[pd.DataFrame, bytes]:
    """
    files_payload: [{'name': str, 'bytes': b'...'}, ...]
    回傳: (combined_long, merged_xlsx_bytes)
    """
    parts = []
...
# 初始化 session_state
if "files_payload" not in st.session_state: st.session_state["files_payload"] = None
if "combined_long" not in st.session_state: st.session_state["combined_long"] = None
if "merged_xlsx" not in st.session_state: st.session_state["merged_xlsx"] = None
...
if combined_long is not None:
    ...
    st.info("請先在側欄上傳 CSV 並按下「清理 + 分群 + 合併」。")

# （可選）若你只想查詢、沒有原始 CSV，也可加這段：
with st.expander("只用 merged_groups_result.xlsx 進行查詢（可選）"):
    merged_up = st.file_uploader("上傳已產生的 merged_groups_result.xlsx", type=["xlsx"], key="merged_uploader")
